...
[x] 3. Actions & Legal Move Generation
  ...
  [x] 3.2 Legal Move Generation
    ...
    [x] 3.2.4 Implement `legal_unlock_actions(state: GameState, player_id: int) -> list[list[UnlockStructureAction]]`: return all legal sequences of structure unlocks affordable given current resources; unlocking slot N costs N Structure tokens; slots must be unlocked sequentially (cannot unlock slot 3 without slot 2).
...
[ ] 4. Game Action Application
  ...
  [ ] 4.6 Apply Unlock Structure Action
    [ ] 4.6.1 Implement `apply_unlock_structure(state: GameState, player_id: int, action: UnlockStructureAction) -> GameState`: validate that `player.structures_unlocked == action.slot - 1` (must be sequential); deduct `action.slot` Structure tokens from resources; increment `player.structures_unlocked`.
  [ ] 4.7 Apply Full Turn
...
[ ] 5. Scoring Engine
  ...
  [ ] 5.7 Headquarters Scoring
    [ ] 5.7.1 Implement `score_headquarters(player: PlayerState, board: PlayerBoardStructure, state: GameState) -> int`: if slot 4 (Headquarters) is not unlocked (`player.structures_unlocked < 4`), return 0; otherwise compute the condition-specific score:
      - Board 1 ("per deployed Structure"): return `player.structures_unlocked` (count of unlocked slots).
